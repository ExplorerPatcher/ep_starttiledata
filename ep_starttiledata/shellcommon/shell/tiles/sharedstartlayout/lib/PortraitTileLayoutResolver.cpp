...
HRESULT CPortraitTileLayoutResolver::_FindTargetDestinationForNewSize2(
    REFGUID itemId, const SIZE& sizeItemCells, Geometry::CRect* rcItemBounds)
{

    HRESULT hr = _spCellArrayManager->GetItemBounds(itemId, *rcItemBounds);
    if (hr >= 0 && (rcItemBounds->GetWidth() != sizeItemCells.cx || rcItemBounds->GetHeight() != sizeItemCells.cy))
    {
        hr = _spCellArrayManager->AddIgnoredItem(itemId);
        if (hr >= 0)
        {
            ...
            Geometry::CRect rcTentativeBounds = Geometry::CRect(rcItemBounds->GetTopLeft(), sizeItemCells);
            if (_IsValidAndEmptyTargetRect(rcTentativeBounds))
            {
                fFound = true;
            }
            ...
            if (!fFound && (rcItemBounds->GetWidth() >= sizeItemCells.cx || rcItemBounds->GetHeight() >= sizeItemCells.cy))
            {
                rcTentativeBounds = Geometry::CRect(rcItemBounds->GetTopLeft(), sizeItemCells);
                fFound = true;
            }
            if (!fFound)
            {
