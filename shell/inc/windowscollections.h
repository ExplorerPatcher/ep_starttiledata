// Downloaded from https://github.com/microsoft/microsoft-ui-xaml/blob/69097129a853c65a16447aade4c82576d4724b1a/src/dxaml/xcp/components/base/inc/windowscollections.h
// Applied some modifications to re-add the stripped AgileVector, and to make it compile with current toolchain.
...
// @MOD
namespace Windows { namespace Internal { namespace Details
{
    class Git
    {
    public:
        ...
        HRESULT Acquire()
        {
            ...
            {
                hr = S_OK;
                InterlockedIncrement(&_cAcquired);
            }
            else
            {
                ...
                if (SUCCEEDED(hr))
                {
                    *(IGlobalInterfaceTable**)std::addressof(spGit) = InterlockedCompareExchangePointer((void**)std::addressof(_spGit), spGit.Get(), nullptr) != nullptr ? spGit.Get() : nullptr;
                    InterlockedIncrement(&_cAcquired);
                }
            }
        ...
        void Release()
        {
            if (InterlockedDecrement(&_cAcquired) == 0)
            {
                _spGit.Reset();
            }
        }
    ...
    private:
        ULONG _cAcquired;
        Microsoft::WRL::ComPtr<IGlobalInterfaceTable> _spGit;
    };

    DECLSPEC_SELECTANY Git _git;
}}}
...
namespace XWinRT
{
    ...
    struct IntVersionTag
    {
        ...
        bool operator==(const IntVersionTag& other) const // @MOD Fix compile: added const
        {
    ...
    template <
        class T,
        class LifetimeTraits = Windows::Foundation::Collections::Internal::DefaultLifetimeTraits<T> // @MOD Don't use wfci_ namespace alias
        >
    class AutoValue
    {
    public:
    ...
    namespace detail
    {
        ...
        public:
            ...
            {
                return AutoValue<T_reftype, LifetimeTraits>::get(); // @MOD Addded template args to fix compile
            }
            ...
            HRESULT Attach(const T_srctype& value)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    AutoValue<T_reftype, LifetimeTraits>::Attach(tmpValue); // @MOD Added template args to fix compile
                }
                return hr;
            }
        ...
        template<
            class T_abi,
            class T_storage,
            class LifetimeTraits = Windows::Foundation::Collections::Internal::DefaultLifetimeTraits<T_abi> // @MOD Don't use wfci_ namespace alias
            >
        ...
        // @MOD BEGIN Restore stripped stuff
        template <typename T>
        class GitStorageType
        {
            class ReferencedGitCookie
            {
            public:
                ReferencedGitCookie() : _refs(1), _gitCookie(0)
                {
                }
                ...
                ULONG Release()
                {
                    ULONG val = InterlockedDecrement(&_refs);
                    if (val == 0 && this)
                    {
                        delete this;
                    }
                ...
                HRESULT Initialize(T* pObject)
                {
                    _ASSERT(_gitCookie == 0);
                    HRESULT hr = Windows::Internal::Details::_git.Acquire();
                    if (SUCCEEDED(hr))
                    {
                        hr = Windows::Internal::Details::_git->RegisterInterfaceInGlobal(pObject, __uuidof(T), &_gitCookie);
                    }
                    return hr;
                }

                HRESULT Localize(T** ppObject)
                {
                    _ASSERT(_gitCookie != 0);
                    *ppObject = nullptr;
                    return Windows::Internal::Details::_git->GetInterfaceFromGlobal(_gitCookie, IID_PPV_ARGS(ppObject));
                }
            ...
            private:
                ~ReferencedGitCookie()
                {
                    if (_gitCookie)
                    {
                        Windows::Internal::Details::_git->RevokeInterfaceFromGlobal(_gitCookie);
                    }
                }
        ...
        public:
            GitStorageType() : _pRefCookie(nullptr), _fGit(false)
            {
            }

            void Duplicate(GitStorageType src)
            {
                _fGit = src._fGit;

                if (src._fGit)
                {
                    _pRefCookie = src._pRefCookie;
                    _pRefCookie->AddRef();
                }
                else
                {
            ...
            HRESULT Initialize(T* pObject)
            {
                ...
                Microsoft::WRL::ComPtr<IAgileObject> agileObj;
                if (SUCCEEDED(pObject->QueryInterface(IID_PPV_ARGS(&agileObj))))
                {
                    _fGit = false;
                    _pObject = pObject;
                    pObject->AddRef();
                ...
                {
                    ReferencedGitCookie* pRefCookie = new(std::nothrow) ReferencedGitCookie();
                    hr = pRefCookie ? S_OK : E_OUTOFMEMORY;
                    if (SUCCEEDED(hr))
                    {
                        hr = pRefCookie->Initialize(pObject);
                        if (SUCCEEDED(hr))
                        {
                            _pRefCookie = pRefCookie;
                            _fGit = true;
                        }
                        else
                        {
            ...
            HRESULT Acquire(T** ppObject)
            {
                HRESULT hr = S_OK;
                if (_fGit)
                {
                    hr = _pRefCookie->Localize(ppObject);
                }
                else
                {
                    *ppObject = _pObject;
        ...
        private:
            union
            {
                ReferencedGitCookie* _pRefCookie;
                T* _pObject;
            };
            bool _fGit;
        };
        // @MOD END Restore stripped stuff
    }
}
...
namespace XWinRT
{
    ...
    struct InterfaceLifetimeTraits
    {
        ...
        // @MOD BEGIN Restore stripped stuff
        template <typename T>
        struct AgileStorageType
        {
            typedef XWinRT::detail::GitStorageType<T> type;
        };

        template <typename T>
        struct AgileTempStorageType
        {
            typedef XWinRT::detail::GitStorageType<T> type;
        };

        template <class TInterface>
        static HRESULT Construct(XWinRT::detail::GitStorageType<TInterface>* uninitializedElement)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            new(uninitializedElement) XWinRT::detail::GitStorageType<TInterface>();
            return S_OK;
        }

        template <class TInterface>
        static HRESULT Construct(XWinRT::detail::GitStorageType<TInterface>* uninitializedElement, TInterface* source)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            new(uninitializedElement) XWinRT::detail::GitStorageType<TInterface>();
            return uninitializedElement->Initialize(source);
        }

        template <class TInterface>
        static HRESULT Construct(XWinRT::detail::GitStorageType<TInterface>* uninitializedElement, const XWinRT::detail::GitStorageType<TInterface>& source)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            uninitializedElement->Duplicate(source);
            return S_OK;
        }

        template <class TInterface>
        static HRESULT Construct(TInterface** uninitializedElement, const XWinRT::detail::GitStorageType<TInterface>& source)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            return const_cast<XWinRT::detail::GitStorageType<TInterface>&>(source).Acquire(uninitializedElement);
        }

        template <class TInterface>
        static HRESULT Move(TInterface** uninitializedElement, XWinRT::detail::GitStorageType<TInterface>* pTemp)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            return pTemp->MoveInto(uninitializedElement);
        }

        template <class TInterface>
        static bool RequiresTempCopy(XWinRT::detail::GitStorageType<TInterface>& initializedElement)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            return initializedElement.IsGitBacked();
        }

        template <class TInterface>
        static void Destroy(XWinRT::detail::GitStorageType<TInterface>* initializedElement)
        {
            static_assert(XWinRT::FakeStl::is_base_of<IUnknown, TInterface>::value,
                          "DefaultLifetimeTraits only supports interface derived from IUnknown");
            initializedElement->Destroy();
        }
        // @MOD END Restore stripped stuff
    };
...
namespace Windows { namespace Foundation { namespace Collections { namespace Internal
{
    namespace detail
    {
        ...
        template <class T, class LifetimeTraits>
        // <IUnknown, DefaultLifetimeTraits<IUnknown*>>
        struct BaseInterfaceStorageType
        {
            using type = typename LifetimeTraits::template AgileStorageType<T>::type;
            using temp_type = typename LifetimeTraits::template AgileTempStorageType<T>::type;
        };

        template <class T>
        struct BasePointerStorageType
        {
            using type = T;
            using temp_type = T;
        };

        template <class LifetimeTraits, typename T>
        struct DefaultAgileTempStorageType
        {
            using type = BasePointerStorageType<T>;
        };

        template <class LifetimeTraits, typename T>
        struct DefaultAgileTempStorageType<LifetimeTraits, T*>
        {
            template <class LifetimeTraitsI, typename TI, typename b>
            struct GetType
            {
                typedef typename _if<b::value,
                    BaseInterfaceStorageType<TI, LifetimeTraitsI>,
                    BasePointerStorageType<TI*>>::type type;
            };

            using type = typename GetType<LifetimeTraits, T, typename XWinRT::FakeStl::is_base_of<IUnknown, T>::type>::type;
        };

        // T = IUnknown*, LifetimeTraits = DefaultLifetimeTraits<IUnknown*>
        template <class T, class LifetimeTraits = DefaultLifetimeTraits<T>, bool isAgile = false>
        struct StorageType;

        template <class T, class LifetimeTraits>
        struct StorageType<T, LifetimeTraits, false> : public BasePointerStorageType<T>
        {
        };

        template <class T, class LifetimeTraits>
        struct StorageType<T, LifetimeTraits, true> : public DefaultAgileTempStorageType<LifetimeTraits, T>::type
        {
        };
        // @MOD END Restore stripped stuff
...
#if 1 || defined(MIDL_NS_PREFIX) || defined(XAML_USE_PUBLIC_SDK) // @MOD Always use ABI::
    ...
    template <class T, bool supportsObservable, bool supportsAgile, bool propagateError = false> // @MOD Restored supportsAgile
    struct VectorOptions
    {
        static const bool hasObservable = false;
        static const bool isAgile = supportsAgile; // @MOD Restored isAgile
    ...
    template <class T, bool supportsAgile, bool propagateError> // @MOD Restored supportsAgile
    struct VectorOptions<T, true, supportsAgile, propagateError>
    {
    ...
    template <class T>
    struct DefaultVectorOptions : VectorOptions<T, false, false, false> // @MOD Restored supportsAgile
    {
    };
    ...
    class Vector : public
        detail::_if <Options::hasObservable,
            typename detail::_if <Options::isAgile,
                Microsoft::WRL::RuntimeClass<Microsoft::WRL::Implements<wfc::IVector<T>, wfc::IIterable<T>, wfc::IObservableVector<T>>, Microsoft::WRL::FtmBase>,
                Microsoft::WRL::RuntimeClass<wfc::IVector<T>, wfc::IIterable<T>, wfc::IObservableVector<T>>
                >::type,
            typename detail::_if <Options::isAgile,
                Microsoft::WRL::RuntimeClass<wfc::IVector<T>, wfc::IIterable<T>, Microsoft::WRL::FtmBase>,
            ...
            >::type
    {
        using base = typename detail::_if <Options::hasObservable,
            ...
            typename detail::_if <Options::isAgile,
                ...
                Microsoft::WRL::RuntimeClass<wfc::IVector<T>, wfc::IIterable<T>>
                >::type
            >::type; // @MOD Restored superclass variants for isAgile
        ...
        typedef typename detail::StorageType<T_abi, LifetimeTraits, Options::isAgile>::type        T_storage; // @MOD Agile support
        typedef typename detail::StorageType<T_abi, LifetimeTraits, Options::isAgile>::temp_type   T_tmp; // @MOD Agile support
    ...
    template <
        ...
        class LifetimeTraits = DefaultLifetimeTraits<T>
    >
    class ObservableVector WrlSealed : public Vector < T, EqualityPredicate, LifetimeTraits, VectorOptions <T, true, false> > // @MOD Restored VectorOptions
    {
        using base = Vector<T, EqualityPredicate, LifetimeTraits, VectorOptions<T, true, false>>; // @MOD Restored VectorOptions
    ...
    template <
        ...
        bool unk = false
    >
    class AgileVector WrlSealed : public Vector < T, EqualityPredicate, LifetimeTraits, VectorOptions <T, false, true> > // @MOD Restored VectorOptions
    {
        using base = Vector<T, EqualityPredicate, LifetimeTraits, VectorOptions<T, false, true>>; // @MOD Restored VectorOptions

    private:
        AgileVector();
    public:
        static HRESULT Make(const EqualityPredicate& fnEquals, _COM_Outptr_ AgileVector** ppVector)
        {
            // The cast in the following line works because AgileVector derives *only* from Vector and
            //   does not contain any data members. Any change to these conditions is bound to break this call
            //   and will result in failures at runtime.
            return base::Make(fnEquals, reinterpret_cast<base**>(ppVector));
        }
        static HRESULT Make(_COM_Outptr_ AgileVector** ppVector)
        {
            return Make(EqualityPredicate(), ppVector);
        }
    };
    ...
    // @MOD BEGIN AgileObservableVector
    template <
        class T,
        class EqualityPredicate = DefaultEqualityPredicate<T>,
        class LifetimeTraits = DefaultLifetimeTraits<T>,
        bool unk = false
    >
    class AgileObservableVector WrlSealed : public Vector < T, EqualityPredicate, LifetimeTraits, VectorOptions <T, true, true> > // @MOD Restored VectorOptions
    {
        using base = Vector<T, EqualityPredicate, LifetimeTraits, VectorOptions<T, true, true>>; // @MOD Restored VectorOptions

    private:
        AgileObservableVector();
    public:
        static HRESULT Make(const EqualityPredicate& fnEquals, _COM_Outptr_ AgileObservableVector** ppVector)
        {
            // The cast in the following line works because AgileObservableVector derives *only* from Vector and
            //   does not contain any data members. Any change to these conditions is bound to break this call
            //   and will result in failures at runtime.
            return base::Make(fnEquals, reinterpret_cast<base**>(ppVector));
        }
        static HRESULT Make(_COM_Outptr_ AgileObservableVector** ppVector)
        {
            return Make(EqualityPredicate(), ppVector);
        }
    };
    // @MOD END AgileObservableVector
    ...
    struct NoVersionTag; // @MOD Forward declared
    ...
    template <
        ...
        >
    class SimpleVectorView; // @MOD Forward declared
    ...
    template <
        ...
        class VersionTag = NoVersionTag
        >
    class SimpleVectorIterator; // @MOD Forward declared
    ...
    template <
        ...
        class TVector/* = wfc::IVector<T>*/,
        class LifetimeTraits/* = DefaultLifetimeTraits<T>*/,
        class VersionTag/* = NoVersionTag*/ // @MOD Was forward declared
        >
...
namespace Windows { namespace Foundation { namespace Collections { namespace Internal {
    ...
    {
        using RuntimeClassT = typename Microsoft::WRL::RuntimeClass<wfc::IMapChangedEventArgs<K>>::RuntimeClassT; // @MOD Fix compile error
    ...
    template <class K, class V, class KeyLifetimeTraits, bool supportsObservable, bool supportsAgile, bool propagateError = false> // @MOD Restored supportsAgile
    ...
    template <class K, class V, class KeyLifetimeTraits, bool supportsAgile, bool propagateError> // @MOD Restored supportsAgile
    struct HashMapOptions<K, V, KeyLifetimeTraits, true, supportsAgile, propagateError>
    {
        static const bool hasObservable = true;
        static const bool isAgile = supportsAgile; // @MOD Restored isAgile
    ...
    struct DefaultHashMapOptions : HashMapOptions<K, V, KeyLifetimeTraits, false, false, false> // @MOD Restored supportsAgile
    {
    };
    ...
    class HashMap : public
        detail::_if <HashMapOptions::hasObservable,
            typename detail::_if <HashMapOptions::isAgile,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>, wfc::IObservableMap<K, V>, Microsoft::WRL::FtmBase>,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>, wfc::IObservableMap<K,V>>
                >::type,
            typename detail::_if <HashMapOptions::isAgile,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>, Microsoft::WRL::FtmBase>,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>>
                >::type
            >::type
    {
        using base = typename detail::_if <HashMapOptions::hasObservable,
            typename detail::_if <HashMapOptions::isAgile,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>, wfc::IObservableMap<K, V>, Microsoft::WRL::FtmBase>,
                Microsoft::WRL::RuntimeClass<wfc::IMap<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*>, wfc::IObservableMap<K,V>>
                >::type,
        ...
        using RuntimeClassT = typename base::RuntimeClassT; // @MOD Fix compile error + Restored superclass variants for isAgile
        ...
        typedef typename detail::StorageType<K_abi, KeyLifetime, HashMapOptions::isAgile>::type               K_storage;  // @MOD Agile support
        typedef typename detail::StorageType<V_abi, ValueLifetime, HashMapOptions::isAgile>::type             V_storage;  // @MOD Agile support
        typedef typename detail::StorageType<V_abi, ValueLifetime, HashMapOptions::isAgile>::temp_type        V_tmp;  // @MOD Agile support
    ...
    class ObservableHashMap WrlSealed : public HashMap <
            ...
            KeyLifetime,
            ValueLifetime,
            HashMapOptions < K, V, KeyLifetime, true, false > > // @MOD Restored HashMapOptions
    {
        using base = HashMap<
            K,
            V,
    ...
    // @MOD BEGIN AgileHashMap
    template <
        typename K,
        typename V,
        typename Hasher = DefaultHash<K>,
        typename EqualityPredicate = DefaultEqualityPredicate<K>,
        typename KeyLifetime = DefaultLifetimeTraits<K>,
        typename ValueLifetime = DefaultLifetimeTraits<V>,
        bool unk = false
    >
    class AgileHashMap WrlSealed : public HashMap < K, V, Hasher, EqualityPredicate, KeyLifetime, ValueLifetime, HashMapOptions<K, V, KeyLifetime, false, true > > // @MOD Restored HashMapOptions
    {
        using base = HashMap<K, V, Hasher, EqualityPredicate, KeyLifetime, ValueLifetime, HashMapOptions<K, V, KeyLifetime, false, true>>; // @MOD Restored HashMapOptions

    private:
        AgileHashMap();
    public:
        static HRESULT Make(const Hasher& fnHasher, const EqualityPredicate& fnEquals, _COM_Outptr_ AgileHashMap** ppHashMap)
        {
            // The cast in the following line works because AgileVector derives *only* from Vector and
            //   does not contain any data members. Any change to these conditions is bound to break this call
            //   and will result in failures at runtime.
        ...
        static HRESULT Make(_COM_Outptr_ AgileHashMap** ppHashMap)
        {
            return Make(Hasher(), EqualityPredicate(), ppHashMap);
        }
    };
    // @MOD END AgileHashMap

    // @MOD BEGIN AgileObservableHashMap
    template <
        typename K,
        typename V,
        typename Hasher = DefaultHash<K>,
        ...
        bool unk = false
    >
    class AgileObservableHashMap WrlSealed : public HashMap < K, V, Hasher, EqualityPredicate, KeyLifetime, ValueLifetime, HashMapOptions<K, V, KeyLifetime, true, true > > // @MOD Restored HashMapOptions
    {
        using base = HashMap<K, V, Hasher, EqualityPredicate, KeyLifetime, ValueLifetime, HashMapOptions<K, V, KeyLifetime, true, true>>; // @MOD Restored HashMapOptions

    private:
        AgileObservableHashMap();
    public:
        static HRESULT Make(const Hasher& fnHasher, const EqualityPredicate& fnEquals, _COM_Outptr_ AgileObservableHashMap** ppHashMap)
        {
            // The cast in the following line works because AgileVector derives *only* from Vector and
            //   does not contain any data members. Any change to these conditions is bound to break this call
            //   and will result in failures at runtime.
        ...
        static HRESULT Make(_COM_Outptr_ AgileObservableHashMap** ppHashMap)
        {
            return Make(Hasher(), EqualityPredicate(), ppHashMap);
        }
    };
    // @MOD END AgileObservableHashMap
        ...
        ::Lookup(_In_opt_ K_abi key, _Out_ V_abi *value)
    {
        ...
        typename V_TempTraits::type tempValue; // @MOD Fix compile
        ...
        ::Insert(_In_opt_ K_abi key, _In_opt_ V_abi value, _Out_ boolean *replaced)
    {
        ...
        *replaced = false;
        ...
        typename HashMapOptions::ReentrancyGuard guard; // @MOD Fix compile
    ...
    {
        using RuntimeClassT = typename Microsoft::WRL::RuntimeClass< wfc::IMapView<K, V>, wfc::IIterable<wfc::IKeyValuePair<K, V>*> >::RuntimeClassT; // @MOD Fix compile error
    ...
    template <
        ...
        class KeyLifetime = DefaultLifetimeTraits<K>,
        class ValueLifetime = DefaultLifetimeTraits<V>,
        bool isAgile = false // @MOD Restored isAgile
    >
    class SimpleKeyValuePair WrlSealed : public Microsoft::WRL::RuntimeClass<wfc::IKeyValuePair<K,V>>
    {
        using RuntimeClassT = typename Microsoft::WRL::RuntimeClass<wfc::IKeyValuePair<K,V>>::RuntimeClassT; // @MOD Fix compile error

        typedef typename detail::GetAbiType<typename wfc::IMap<K,V>::K_complex>::type       K_abi;
        typedef typename detail::GetAbiType<typename wfc::IMap<K,V>::V_complex>::type       V_abi;
        typedef typename detail::StorageType<K_abi, KeyLifetime, isAgile>::type             K_storage; // @MOD Restored isAgile
        typedef typename detail::StorageType<V_abi, ValueLifetime, isAgile>::type           V_storage; // @MOD Restored isAgile
    ...
    {
        using RuntimeClassT = typename Microsoft::WRL::RuntimeClass<wfc::IIterator<wfc::IKeyValuePair<K, V>*>>::RuntimeClassT; // @MOD Fix compile error
            ...
            *current = nullptr;
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    Microsoft::WRL::ComPtr<SimpleKeyValuePair<K, V, KeyLifetime, ValueLifetime, HashMapOptions::isAgile>> spPair; // @MOD Restored isAgile
                    hr = SimpleKeyValuePair<K, V, KeyLifetime, ValueLifetime, HashMapOptions::isAgile>::Make( // @MOD Restored isAgile
                        pCurPair->m_key,
                        pCurPair->m_value,
                        &spPair);
                    if (SUCCEEDED(hr))
